...
namespace Angularintigration.Repositories
{
    public class RegisterRepository : IRegistrationRepositry
    {
        ...
        public async Task<int> RegisterUser(RegistrationPage model)
        {
            ...
            var checkQuery = @"SELECT COUNT(1) FROM vinod.users WHERE email = @Email";

            var exists = await connection.ExecuteScalarAsync<int>(checkQuery,new { Email = model.Email });

            if (exists > 0)
                 return -1;
            

            var query = @"insert into vinod.users (fullname,email,mobile,password,roleid) values (@FullName,@Email,@Mobile,@Password,@RoleId)";
